...
Не пиши .md документы с отчетами после завершения задачи - это бесполезно. Комментарии в коде тоже не надо писать. Главное - чистый код и понятный нейминг.
Для логирования на Go используй slog. В сервисе должен быть одинаковый формат логгирования.
...
### Users
Частные инвесторы среднего уровня на российском рынке (MOEX). Имеют базовые знания об акциях и финансовых показателях, но не профессионалы. Ищут инструмент, который поможет принимать обоснованные инвестиционные решения на основе фундаментального анализа с помощью AI. Контекст использования: десктоп-первый (анализ отчетов, дашборды), но мобильная версия тоже важна для быстрого мониторинга.

### Brand Personality
**Надежный, умный, простой.** Серьезный финтех-инструмент, вызывающий доверие. Не кричащий стартап и не сухой терминал — сбалансированный профессионализм. Тон: уверенный эксперт, объясняющий сложное простым языком.

### Aesthetic Direction
- **Визуальный тон:** Чистый минимализм в стиле Linear/Vercel — много воздуха, четкая типографика, сдержанная палитра с точечными акцентами.
- **Тема:** Light + Dark (оба режима поддерживаются, dark как основной для работы с данными).
- **Цвета:** oklch-палитра на hue 240 (сине-серая нейтраль). Минимум ярких цветов, акценты только для ключевых действий и данных.
- **Типография:** Geist Sans (основной) + Geist Mono (данные/числа). Чёткая иерархия заголовков.
- **Анти-референсы:** НЕ должен выглядеть как типичный AI-продукт (без gradient text, glassmorphism, hero metrics grid, rainbow палитр). НЕ как перегруженный Bloomberg Terminal. НЕ как яркий Tinkoff с жёлтыми акцентами.

### Design Principles
1. **Данные на первом месте** — интерфейс служит данным, не наоборот. Каждый элемент должен помогать пользователю быстрее понять финансовую информацию.
2. **Уверенность через ясность** — пользователь должен чувствовать контроль. Никакой перегрузки, нет двусмысленности. Чёткие состояния, предсказуемая навигация.
3. **Минимум шума** — убрать всё лишнее. Если элемент не помогает принять решение — он не нужен. Предпочитать whitespace декору.
4. **Консистентность** — использовать shadcn/ui токены и компоненты системно. Одинаковые паттерны для одинаковых задач.
5. **Профессиональная достоверность** — визуал должен внушать доверие. Никаких мультяшных иллюстраций, анимаций ради анимаций, или модных AI-трендов.

## Полезные команды

`docker exec postgres_financial psql -U postgres -d trade-compass -c "DELETE FROM metrics WHERE ticker = 'T';"` - очистить все raw data по тикеру
`make ai` - пересобрать ai-service, нужно вызывать после любых изменений для деплоя в прод
