...
func (r *CBRateRepository) GetCurrent(ctx context.Context) (*domain.CBRate, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, fmt.Errorf("no CB rates found: %w", domain.ErrNotFound)
		}
...
func (r *CBRateRepository) GetByDate(ctx context.Context, date time.Time) (*domain.CBRate, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, fmt.Errorf("CB rate not found for date %s: %w", date.Format("2006-01-02"), domain.ErrNotFound)
		}
...
func (r *CBRateRepository) GetHistory(ctx context.Context, from, to time.Time) ([]domain.CBRate, error) {
	...
	if len(rates) == 0 {
		return nil, fmt.Errorf("no CB rates found for the specified period: %w", domain.ErrNotFound)
	}
...
func (r *CBRateRepository) Create(ctx context.Context, rate *domain.CBRate) error {
	if rate == nil {
		return fmt.Errorf("rate is nil: %w", domain.ErrInvalidInput)
	}
...
func (r *CBRateRepository) Update(ctx context.Context, date time.Time, rate float64) error {
	...
	if result.RowsAffected() == 0 {
		return fmt.Errorf("CB rate not found for date %s: %w", date.Format("2006-01-02"), domain.ErrNotFound)
	}
