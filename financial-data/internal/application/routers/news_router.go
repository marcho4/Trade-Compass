...
func RegisterNewsRoutes(r chi.Router, repo NewsRepository, m *middleware.MiddlewareConfig) {
	...
	r.Group(func(protected chi.Router) {
		...
		protected.Post("/news", handler.HandleCreate)
		protected.Put("/news/{id}", handler.HandleUpdate)
		protected.Delete("/news/{id}", handler.HandleDelete)
	})
}
...
func (h *NewsHandler) HandleGetNews(w http.ResponseWriter, r *http.Request) {
	...
	if ticker != "" {
		...
		if parseErr != nil {
			response.RespondWithError(w, r, 400, "invalid sector_id", parseErr)
			return
		}
		...
		response.RespondWithError(w, r, 400, "ticker or sector query parameter is required", nil)
		return
	}

	if err != nil {
		response.RespondWithError(w, r, 500, "failed to load news", err)
		return
	}
...
func (h *NewsHandler) HandleCreate(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.repo.Create(r.Context(), &news); err != nil {
		response.RespondWithError(w, r, 500, "failed to create news", err)
		return
	}

	response.RespondWithSuccess(w, 201, news, "News successfully created")
}
...
func (h *NewsHandler) HandleUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.repo.Update(r.Context(), id, &news); err != nil {
		response.RespondWithError(w, r, 500, "failed to update news", err)
		return
	}

	response.RespondWithSuccess(w, 200, map[string]string{"status": "updated"}, "News successfully updated")
}
...
func (h *NewsHandler) HandleDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.repo.Delete(r.Context(), id); err != nil {
		response.RespondWithError(w, r, 500, "failed to delete news", err)
		return
	}

	response.RespondWithSuccess(w, 204, nil, "News successfully deleted")
}
