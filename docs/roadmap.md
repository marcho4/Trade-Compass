...
- [x] Создать основную структуру монорепозитория
- [x] Настроить Docker Compose для локальной разработки
- [x] Настроить PostgreSQL локально и в Docker
- [x] Создать .env файлы для конфигурации
- [x] Настроить Makefile для общих команд

### 0.2 Инициализация сервисов

- [x] Создать структуру Go сервиса (metrics-service)
- [x] Создать структуру Next.js приложения (frontend)
- [x] Улучшить структуру Python сервиса (parser)
- [x] Настроить .gitignore для всех сервисов

### 0.3 CI/CD базовый

- [x] Создать GitHub Actions для деплоя и сборки
- [ ] Настроить базовые тесты для каждого сервиса
- [x] Подготовить deployment скрипты для Hetzner (без деплоя)
...
- [x] Создать парсер для получения списка отчетов по ИНН
- [x] Скачивание PDF/XBRL файлов
- [x] Сохранение в объектное хранилище (S3-compatible, например Backblaze B2)
- [x] Запись метаданных в таблицу reports
...
2. Phase 1.3 - калькулятор метрик
...
2. Phase 7.1 - деплой на прод

### Далее

9. Phase 5 - автоматический парсер
2. Phase 6 - расширенные фичи
3. Phase 7 - оптимизация
4. Phase 8 - маркетинг
